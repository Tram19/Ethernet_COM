...
#ifdef GREMSY_MODULE_PROTOCOL_MAVLINK_V100
...
StatusEnum Module::Protocol::Mavlink::TransmittedMessageHeartbeat(void)
{
    ...
    if(txMessageHeartbeatTime >= 1000000)
    {
        ...
        uint8_t             sysid = systemId;
        uint8_t             compid = compId;
        ...
        if(funcTransmittedMessageHeartbeatCallback)
        {
            ...
            /// compid = funcTransmittedMessageHeartbeatCallbackArg->compId;  /// Khong can chuyen doi compid cua thiet bi
        }
...
StatusEnum Module::Protocol::Mavlink::Open(void)
{
    ...
    /// CHECK
    if(name.IsExist() == ERROR_NOT_EXIST || compId == 0 || systemId == 0)
    {
