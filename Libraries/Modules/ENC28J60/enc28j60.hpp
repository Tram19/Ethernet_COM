...
#include "Modules/Base/module_base.hpp"
#include <mutex>
...
#define ENC28J60_CS_PORT                                        ETH_SPI_CS_Pin
#define ENC28J60_CS_PIN                                         ETH_SPI_CS_GPIO_Port
...
#define ENC28J60_TX_BUF_START                                   (0x1FFF - 0x0600)  
#define ENC28J60_RX_BUF_START                                   0x0000 // errata number 5, Sometimes, when ERXST or
                                                                      // ERXND is written to, the exact value, 0000h, is
                                                                      // stored in the internal receive Write Pointer instead
                                                                      // of the ERXST address.
#define ENC28J60_RX_BUF_END                                     (0x1FFF - 0x0600 - 1) 
...
{
public:
  explicit ENC28J60(HW::Peripheral::Spi& spi);
  bool Init();
  //giao tiếp SPI với ENC28J60 để lấy gói tin từ bộ đệm (Receive Buffer) ra ngoài RAM của vi điều khiển.
  uint16_t ReceiveFrame(ENC28J60_Frame*frame);

/*ETXSTL / ETXSTH – địa chỉ của điểm bắt đầu bộ đệm truyền dữ liệu
ETXNDL / ETXNDH – địa chỉ của điểm cuối bộ đệm để truyền dữ liệu
ERXSTL / ERXSTH – địa chỉ của đầu bộ đệm nhận
ERXNDL / ERXNDH – địa chỉ của điểm cuối bộ đệm nhận
Hai thanh ghi cuối cùng được sử dụng để xác định vùng nhớ cho bộ đệm Rx,
trong khi phần còn lại được phân bổ cho bộ đệm Tx theo mặc định. Trong quá trình truyền, chúng ta sẽ ghi địa chỉ của byte bắt đầu và byte kết thúc của khung Ethernet cụ thể mà chúng ta sắp truyền vào các thanh ghi ETXST và ETXND .
*/
  StatusEnum TransmitFrame(uint8_t *data, uint16_t size);
  void StartReceiving();
...
private:
  ...
  bool ReadBufferMem(uint8_t* data, uint16_t size);
  uint16_t ReadPhyReg(uint8_t reg);
  void SystemReset();
  void WritePhyReg(uint8_t reg, uint16_t regData);

  void RescueRx();
  mutable RTOS::StaticMutex _encmutex;
